...
**job_seeker_ro_spider** — scraper pentru job-urile SOBIS AP din România.

Extrage anunțurile de pe [SOBIS AP Cariere](https://sobis-ap.ro/cariere/) și le publică în [peviitor.ro](https://peviitor.ro) prin API-ul SOLR.
...
1. **Validează compania** — interoghează API-ul public ANAF ([demoanaf.ro](https://demoanaf.ro)) după CIF-ul SOBIS AP (52200796) și verifică:
   - Denumirea oficială: SOBIS AP S.R.L.
...
6. **Generează docs/jobs.md** — fișier markdown cu informații companie + toate job-urile curente, publicat pe [GitHub Pages](https://sebiboga.github.io/sobis-ap-srl-nodejs-scraper/jobs.md)
...
├── index.js                    # Orchestrator principal (HTML/cheerio)
...
├── ROBOTS.md                   # Politici de scraping pentru SOBIS AP
├── tests/
...
|---|---|---|
| SOBIS AP Cariere | `https://sobis-ap.ro/cariere/` | Public (HTML) |
...
SOBIS AP careers page nu are un robots.txt dedicat. Pagina este HTML static, accesibilă direct. Scraperul folosește User-Agent standard `job_seeker_ro_spider`.
...
# Doar E2E (pagină reală SOBIS AP + ANAF + SOLR)
npm run test:e2e
```
