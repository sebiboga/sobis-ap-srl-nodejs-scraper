...
| `src/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by both `validate-jobs.js` and `tests/validate-sobis-ap-jobs.js`. |
...
| `tests/company.json` | Mock ANAF company data for SOBIS AP used in unit tests |
| `tests/validate-sobis-ap-jobs.js` | **SOBIS AP fast validator (used by CI).** HEAD requests only, CIF from config. Called nightly by `automation-testing.yml`. Supports `--dry-run` and `--delete`. |
| `tests/unit/index.test.js` | Unit tests for index.js - parseCareersHtml, mapToJobModel, transformJobsForSOLR |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real SOBIS AP page, ANAF, and SOLR |
...
| `ROBOTS.md` | Scraping policy for SOBIS AP careers page |
...
| `delete_request.json` | **Manual maintenance tool** — SOLR payload to delete ALL jobs for CIF 52200796. Use only when you need to wipe SOBIS AP jobs from SOLR entirely. |
...
- Full workflow: validate company (ANAF+Peviitor) → scrape SOBIS AP → transform → upsert SOLR → generate docs/jobs.md
