...
This scraper extracts job listings from SOBIS AP S.R.L. careers page (HTML/cheerio) and imports them to peviitor.ro.

Target: https://sobis-ap.ro/cariere/
...
- **cheerio** - For HTML parsing of careers page
- **Apache SOLR** - For data storage and indexing
- **Claude Code** - For development

## Workflow Steps

1. **Start with brand** - We know the brand (e.g., "SOBIS AP")
...
8. **Scrape new jobs** - Extract jobs from SOBIS AP careers page (HTML)
...
> **Important**: Scraper does NOT delete jobs from other sources (ANOFM, etc). It only upserts SOBIS AP Careers jobs. Existing jobs are preserved.
...
3. **Scrape jobs** - Extract jobs from SOBIS AP careers page (HTML/cheerio)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape SOBIS AP careers page (HTML/cheerio)
    │
    ▼
...
| `tests/validate-sobis-ap-jobs.js` | CI fast validator (HEAD only); thin CLI over `src/job-validator.js` + `solr.js` |
| `tests/unit/index.test.js` | Unit tests for parseCareersHtml, mapToJobModel, transformJobsForSOLR |
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real SOBIS AP careers page |
