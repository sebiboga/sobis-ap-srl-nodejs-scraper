...
describe('solr.js', () => {
  ...
  describe('querySOLR', () => {
    it('should return response object with docs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { id: 'job1', url: 'https://test.com/1', cif: '52200796' },
        { id: 'job2', url: 'https://test.com/2', cif: '52200796' }
      ]));

      const result = await solr.querySOLR('52200796');
    ...
    it('should throw when SOLR_AUTH is missing', async () => {
      delete process.env.SOLR_AUTH;
      await expect(solr.querySOLR('52200796')).rejects.toThrow('SOLR_AUTH not set in environment');
      process.env.SOLR_AUTH = 'test:test';
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('52200796')).rejects.toThrow('SOLR query error: 500');
    });
  });
  ...
  describe('queryCompanySOLR', () => {
    it('should return company data', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { id: '52200796', company: 'SOBIS AP S.R.L.', brand: 'SOBIS AP' }
      ]));

      const result = await solr.queryCompanySOLR('id:52200796');

      expect(result.numFound).toBe(1);
      expect(result.docs[0].brand).toBe('SOBIS AP');
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.queryCompanySOLR('id:52200796')).rejects.toThrow('SOLR company query error: 401');
    });
  });
  ...
  describe('deleteJobsByCIF', () => {
    it('should delete all jobs for a CIF', async () => {
      ...
      await expect(solr.deleteJobsByCIF('52200796')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobsByCIF('52200796')).rejects.toThrow('SOLR delete error: 500');
    });
  });
  ...
  describe('Data Integrity', () => {
    it('should not have duplicate URLs for same CIF', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://test.com/job1', title: 'Job 1', cif: '52200796' },
        { url: 'https://test.com/job2', title: 'Job 2', cif: '52200796' }
      ]));
    ...
    it('should have valid CIF format for all jobs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://test.com/1', title: 'Job 1', cif: '52200796' },
    ...
    it('should have valid status values', async () => {
      ...
      mockFetch.mockResolvedValue(makeSolrResponse(3, [
        { url: 'https://test.com/1', title: 'Job 1', cif: '52200796', status: 'scraped' },
        { url: 'https://test.com/2', title: 'Job 2', cif: '52200796', status: 'verified' },
        { url: 'https://test.com/3', title: 'Job 3', cif: '52200796', status: 'published' }
      ]));
